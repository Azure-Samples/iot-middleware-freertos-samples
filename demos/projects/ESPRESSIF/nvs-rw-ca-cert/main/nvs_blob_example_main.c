...
esp_err_t save_trust_bundle(void)
{
    ...
    err = nvs_get_blob(my_handle, AZURE_TRUST_BUNDLE_NAME, NULL, &required_size);
    ...
    if (required_size > 0) {
        err = nvs_get_blob(my_handle, AZURE_TRUST_BUNDLE_NAME, trust_bundle, &required_size);
        if (err != ESP_OK) {
            return err;
        }
    }
    ...
    err = nvs_set_blob(my_handle, AZURE_TRUST_BUNDLE_NAME, trust_bundle, required_size);
