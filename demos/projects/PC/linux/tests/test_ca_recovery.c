...
int vStartTestTask( void )
{
    ...
    else if( strncmp( ( const char * ) xPayload.xTrustBundle.pucVersion, TEST_BUNDLE_VERSION, sizeof( TEST_BUNDLE_VERSION ) - 1 ) != 0 )
    {
        printf( "\tBundle Version Failed!\n" );
        return TEST_CA_RECOVERY_FAIL;
    }
    else if( xPayload.xTrustBundle.ullExpiryTime != TEST_BUNDLE_EXPIRY )
    {
